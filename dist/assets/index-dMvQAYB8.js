(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))t(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&t(r)}).observe(document,{childList:!0,subtree:!0});function i(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function t(a){if(a.ep)return;a.ep=!0;const s=i(a);fetch(a.href,s)}})();const p=()=>`
  ...
  `},m=()=>{const o=document.getElementById("input-busqueda"),e=o?o.value:"";console.log("Buscando:",e)},f=()=>{const o=document.getElementById("input-busqueda");o&&(o.value=""),document.querySelectorAll(".filtro-select").forEach(i=>{i.value=""}),console.log("Filtros limpiados")},g=()=>{const o=document.querySelector(".btn-buscar"),e=document.querySelector(".btn-limpiar");o&&o.addEventListener("click",m),e&&e.addEventListener("click",f)},b=()=>{const o=document.querySelector("#empresas-container");if(!o)return;const e=h.filter(t=>t.destacada);if(e.length===0){o.innerHTML=`
      <div class="empty-state">
        <p>No hay empresas destacadas disponibles en este momento.</p>
      </div>
    `;return}const i=e.map(t=>v(t)).join("");o.innerHTML=i,g()},w=()=>`
    <footer class="footer">
  ...
  <div class="historias-container">
    ...
    ${w()}
  `,k=[{path:"#/home",view:()=>x(),init:()=>b()}],C=(o,e)=>{const i=o.split("/"),t=e.split("/"),a={};if(i.length!==t.length)return null;for(let s=0;s<i.length;s++){const r=i[s],c=t[s];if(r.startsWith(":"))a[r.substring(1)]=c;else if(r!==c)return null}return a},B=o=>{for(const e of k){const i=C(e.path,o);if(i!=null)return{route:e,params:i}}return{route:null,params:{}}},E=async()=>{const o=window.location.hash||"#/home",{route:e,params:i}=B(o),t=document.querySelector("#app");if(!e){t.innerHTML=`
        <h1>404 - No se encontro la ruta<h1>`;return}const a=e.view();t.innerHTML=a instanceof Promise?await a:a,e.init&&e.init(i)},n=()=>{E()};window.addEventListener("hashchange",n);window.addEventListener("DOMContentLoaded",n);
